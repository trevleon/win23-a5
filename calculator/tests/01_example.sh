...
if $CALCULATOR a + b; then
  echo 'ERROR! An invalid run of the application (a + b) apparently succeeded?!'
  exit 1
fi
...
if [[ $($CALCULATOR 2 - 2) -ne 0]]; then  # If the output of the program is not 2...
  echo 'ERROR! A valid run of the calculator (2 - 2) failed to produce 0 as an output!'
  exit 1
fi

if [[ $($CALCULATOR 2 / 2) -ne 1 ]]; then  # If the output of the program is not 2...
